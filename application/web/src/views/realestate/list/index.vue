<template>
  <div class="container">
    ...
    >
      ...
      <p>用户名: {{ userName }}</p>
      <p>余额: ￥{{ balance }} 元</p>
      <p>当发起能源交易、能源归还时，交易状态为true</p>
      <p>当未被交易为false时，才可发起能源交易、能源归还操作</p>
    </el-alert>
    ...
    <el-row v-loading="loading" :gutter="20">
      <el-col v-for="(val,index) in realEstateList" :key="index" :span="6" :offset="1">
        <el-card class="realEstate-card">
          ...
          <div class="item">
            <el-tag>充换电小柜ID: </el-tag>
            <span>{{ val.realEstateId }}</span>
          </div>
          <div class="item">
            <el-tag type="success">充换电柜服务网点ID: </el-tag>
            <span>{{ val.proprietor }}</span>
          </div>
          <div class="item">
            <el-tag type="warning">额定容量: </el-tag>
            <span>{{ val.totalArea }} ㎡</span>
          </div>
          <div class="item">
            <el-tag type="danger">实际容量: </el-tag>
          ...
          <div v-if="!val.encumbrance&&roles[0] !== 'admin'">
            ...
            <el-button type="text" @click="openDonatingDialog(val)">充换电小柜归还</el-button>
          </div>
    ...
    <el-dialog v-loading="loadingDialog" :visible.sync="dialogCreateSelling" :close-on-click-modal="false" @close="resetForm('realForm')">
      <el-form ref="realForm" :model="realForm" :rules="rules" label-width="100px">
        ...
        <el-form-item label="最大使用时长 (天)" prop="salePeriod">
    ...
    <el-dialog v-loading="loadingDialog" :visible.sync="dialogCreateDonating" :close-on-click-modal="false" @close="resetForm('DonatingForm')">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="createDonating('DonatingForm')">立即充换电小柜归还</el-button>
        <el-button @click="dialogCreateDonating = false">取消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
