      ...
      <el-col v-for="(val,index) in donatingList" :key="index" :span="6" :offset="1">
        <el-card class="d-all-card">
          <div slot="header" class="clearfix">
            ...
            <el-button v-if="roles[0] !== 'admin'&&val.grantee===accountId&&val.donatingStatus==='归还充换电小柜中'" style="float: right; padding: 3px 6px" type="text" @click="updateDonating(val,'done')">确认接收</el-button>
            <el-button v-if="roles[0] !== 'admin'&&(val.donor===accountId||val.grantee===accountId)&&val.donatingStatus==='归还充换电小柜中'" style="float: right; padding: 3px 0" type="text" @click="updateDonating(val,'cancelled')">取消</el-button>
          </div>
          <div class="item">
            <el-tag>充换电柜网点ID: </el-tag>
            <span>{{ val.objectOfDonating }}</span>
          </div>
          <div class="item">
            <el-tag type="success">归还充换电小柜者ID: </el-tag>
            <span>{{ val.donor }}</span>
          </div>
          <div class="item">
            <el-tag type="danger">归还人ID: </el-tag>
